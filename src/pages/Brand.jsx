...
import { CopyToClipboard } from 'react-copy-to-clipboard'; import { toast } from 'react-toastify';
...
function Brand() {
    ...
    return <section className="bg-[#edf1f8]">
        <div className="bg-white">
            <div className="w-11/12 mx-auto">
                <div className="flex max-sm:flex-col justify-center items-center gap-2 p-4">
                    <img className="w-36 h-36 rounded-lg" src={state.brand_logo} alt="" />
                    <div className="space-y-4">
                        <p className="text-4xl font-bold">{state.brand_name} Coupon Codes </p>
                        <span className="text-sm px-4 py-2 bg-[#004e98] text-white rounded-lg font-semibold mr-4">{state.category}</span>
        ...
        <div>
            <p className="w-11/12 mx-auto font-bold text-xl px-2 py-10">Total coupons ({state.coupons.length})</p>
                    ...
                    state.coupons.map(
                        (item, i) => <div className="bg-white p-4 rounded-lg max-w-[45ch] font-semibold flex flex-col justify-evenly gap-4 shadow-sm" key={i}>
                            ...
                            <div className="flex justify-between">
                                ...
                                <Link className="w-[45%] py-2 bg-[#3a6ea5] text-white font-semibold text-center rounded-lg inline-block" to={state.shop_Link} target="_blank">Use Now</Link>
                            </div>
                        </div>
                    )
                }

...
function UpdateProfile() {
    ...
    return <section className="my-10 mx-2 flex max-md:flex-col justify-center items-center gap-4">
        <div className='max-w-[500px] md:max-w-[595px] relative'>
            <img className='rounded-lg' src={cover} alt="" />
            <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
                <p className='text-xl text-center font-semibold max-w-96 overflow-hidden'>Welcome <br /> {user?.displayName || "you"}!</p>
            </div>
        </div>
        <div className="bg-white max-w-[500px] p-8 md:p-20 space-y-8 rounded-lg">
            <div className="text-center space-y-4">
                ...
                <div className="space-y-2">
                    <p className="text-lg font-bold max-w-96 overflow-hidden">{user?.displayName}</p>
            ...
            <form className="space-y-4" onSubmit={e => handleSubmit(e)}>
                ...
                <input className="w-full border p-3 rounded-lg focus:outline-none" type="text" placeholder="Photo URL" name="photo_url" defaultValue={user?.photoURL} />
                    <Link className="text-sm inline-block m-1 hover:underline" to='/forgot_password' state={user.email}>Forgot Password ?</Link>
